...
## Implementation Notes (added post-execution, 2026-07-11)

Plan executed task-by-task with real TDD (write test → verify red → implement → verify green → commit). All 5 tasks committed on `worktree-wallet-core-ports-magic-adapter`; 66/66 tests pass, `tsc --noEmit` clean. Three implementation-level deviations were found and fixed during execution — none change the architecture or any port signature defined above, so no Planning.md/SPEC.md change was needed.

1. **Task 1's "verify it fails" step doesn't actually fail under `vitest run`.** `wallet-port.test.ts` only consumes `WalletPort`/`IdentityAttestation`/`WalletRecord` via `import type`. esbuild (vitest's transform) elides type-only imports per-file without cross-file module resolution, so a missing target module doesn't cause a runtime failure — the test suite reports green with 0 source files present. `tsc --noEmit` *does* catch it (`TS2307: Cannot find module`), so from Task 1 onward the red-verification step in this plan should read "run `tsc --noEmit`," not just `vitest run`, whenever a test's only imports of the new module(s) are `import type`. Tasks 2–5's tests import real values (classes), so `vitest run` alone red-fails correctly for those.

2. **Task 2's route code as originally drafted violated this plan's own Global Constraint** ("routes must keep their exact current external contract ... same status codes"). The original pre-existing routes passed through the Magic TEE's actual HTTP status on failure (e.g. a 429 stayed a 429); the plan's drafted replacement collapsed every adapter failure to 500. Fixed by adding `MagicApiError extends Error` (carries `status`/`detail`) to `magic-wallet-adapter.ts`, thrown instead of a plain `Error`, and both routes now do `if (err instanceof MagicApiError) status = err.status`. Added a covering test per route (`passes through the TEE's original status code on MagicApiError`). `MagicApiError` is adapter-local, not part of the `WalletPort` interface, so this doesn't reintroduce provider types into the port signature.

3. **Task 2's route tests need `vitest.config.ts` alias resolution for `@/`.** The routes use `@/lib/adapters/...` (the established import style — used in 34 existing files, including every other route handler), which resolves fine under `tsc`/Next.js via `tsconfig.json`'s `paths`, but `vitest.config.ts` had no equivalent alias, so `vi.mock("@/lib/...")`-adjacent route tests couldn't resolve the import at all. Fixed by adding `resolve.alias: { "@": path.resolve(__dirname, "./src") }` to `vitest.config.ts`. This is a one-time config fix that benefits every future route test, not just this plan's.

4. **Task 3's `NotImplementedError` had a latent bug surfaced by Task 4's reuse.** Its constructor hardcoded the string `"StubAccountAdapter.${method}"`, so when `StubExecutionAdapter` (Task 4) threw the same shared error class, the message read `"StubAccountAdapter.quote is not implemented"` instead of `"StubExecutionAdapter.quote"` — misleading for anyone debugging a Plan-6-blocked call. Neither task's tests caught it because both only assert `.rejects.toThrow(NotImplementedError)` (class identity), never the message text. Fixed by parameterizing the full `"ClassName.method"` label at each call site instead of hardcoding the class name inside the shared error class.
